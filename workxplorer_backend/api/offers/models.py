...
class OfferStatus(models.TextChoices):
    ...
    ACCEPTED_BY_CUSTOMER = "ACCEPTED_BY_CUSTOMER", "Принято заказчиком (ожидает подтверждения)"
...
class Offer(models.Model):
    ...
    class Meta:
        constraints = [
            # один активный оффер на (cargo, carrier)
            models.UniqueConstraint(
                fields=["cargo", "carrier"],
                condition=Q(status__in=[
                    OfferStatus.PENDING,
                    OfferStatus.COUNTERED_BY_CUSTOMER,
    ...
    def __str__(self):
        return f"Offer#{self.pk} cargo={self.cargo_id} by {self.carrier_id} {self.amount_value} {self.amount_currency} [{self.status}]"
